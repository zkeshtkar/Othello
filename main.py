...
class Board:
    ...
    def alphaBeta(self, node, depth, alpha, beta, maximizing):
        ...
        for x in range(8):
            for y in range(8):
                if valid(self.array, self.player, x, y):
                    test = moveconv(node, x, y)
                    convert.append(test[1])
                    averageconv += test[1]
                    boards.append(test[0])
                    choices.append([x, y])

        if len(convert) != 0:
            averageconv /= len(convert)
            i = 0
            while i < len(convert):
                if convert[i] < averageconv or averageconv == 0:
                    convert.pop(i)
                    boards.pop(i)
                    choices.pop(i)
                else:
        ...
        if depth == 0 or len(choices) == 0:
            return ([finalHeuristic(node, maximizing, len(choices)), node])
...
#
def num_valid_move(array, colour):
    # Generates all possible moves
    if colour == "b":
        player = 1
    else:
